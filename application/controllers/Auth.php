...
class Auth extends CI_Controller {
	...
	{
		$this->template->load(template().'/template',template().'/reseller/view_lupa');
	}
	public function lupass(){
		if (isset($_POST['lupa'])){
			...
			if ($total > 0){
				...
				$data['email'] = $email;
				echo $this->session->set_flashdata('message', '<div class="alert alert-danger"><center>Email tidak ditemukan!!</center></div>');
				$this->template->load(template().'/template',template().'/reseller/view_lupa',$data);
			}
		}
	}
}
